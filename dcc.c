...
static void 
dcc_add_deadclient (register DCC_list * client)
{
	struct deadlist *new;
...
void 
dcc_chat (char *command, char *args)
{
	char *user;
	char *port = NULL;
...
void 
dcc_resend (char *command, char *args)
{
	char *user = NULL;
...
void 
dcc_filesend (char *command, char *args)
{
	char *user = NULL;
	char *filename = NULL, *fullname = NULL;
...
void 
multiget (char *usern, char *filen)
{
	DCC_list *dccList;
	char *newbuf = NULL;
	char *expand = NULL;
...
static void 
dcc_getfile (char *command, char *args)
{
	char *user;
	char *filename = NULL;
	char *tmp = NULL;
...
void 
dcc_regetfile (char *command, char *args)
{
	char *user;
	char *tmp = NULL;
	char *filename;
...
static void 
dcc_send_raw (char *command, char *args)
{
	char *name;
...
static char *
get_dcc_type (unsigned long flag)
{
	static char ret[20];
...
void 
dcc_glist (char *command, char *args)
{
	...
	for (Client = ClientList; Client != NULL; Client = Client->next)
	{
		...
		*kilobytes = 0;
		...
		if (do_hook (DCC_STATF_LIST, "%d %s %s %s %s %s",
			     ...
			     Client->flags & DCC_CNCT_PEND ? "Connect" :
			     "Unknown",
			     kilobytes, check_paths (Client->description)))
		{
			char *s;
...
static void 
output_reject_ctcp (char *notused, char *nicklist)
{
...
static void 
dcc_rename (char *command, char *args)
{
	DCC_list *Client;
	char *user;
	char *description;
...
static void 
add_to_dcc_buffer (DCC_list * Client, char *buf)
{
	if (buf && *buf)
	{
		if (Client->buffer)
...
void
cmd_chat (struct command *cmd, char *args)
{
	int no_chat = 0;
...
static void 
DCC_close_filesend (DCC_list * Client, char *type)
{
...
static void 
update_transfer_buffer (char *format,...)
{
...
static void 
dcc_help1 (char *command, char *args)
{
...
static void 
dcc_set_paths (char *command, char *args)
{
	dcc_paths ^= 1;
...
static void 
dcc_tog_rename (char *command, char *args)
{
...
static void 
dcc_overwrite_toggle (char *command, char *args)
{
	dcc_overwrite_var ^= 1;
