...
typedef struct {
    ...
    int termsig;		/* The signal that terminated the process */
...
static void list_processes(void)
{
	...
	for (i = 0; i < process_list_size; i++) {
	    if ((proc = process_list[i]) != NULL) {
		if (proc->logical)
		    say("\t%d (%s): %s", i, proc->logical, proc->name);
		else
		    say("\t%d: %s", i, proc->name);
	    }
	}
 ...
 */
static void
add_process(char *name, char *logical, int pid, int p_stdin, int p_stdout, int p_stderr, char *redirect, char *who,
	    unsigned int refnum)
{
    int i;
    Process *proc;
...
static int is_logical_unique(char *logical)
{
    ...
    if (logical) {
	for (i = 0; i < process_list_size; i++)
	    if ((proc = process_list[i]) && proc->logical && (my_stricmp(proc->logical, logical) == 0))
		return 0;
    }
    return 1;
}
...
int logical_to_index(char *logical)
{
    ...
    for (i = 0; i < process_list_size; i++) {
	if ((proc = process_list[i]) && proc->logical && (!my_stricmp(proc->logical, logical)))
	    return i;
    }
    return -1;
}
 ...
 */
void cmd_exec(struct command *cmd, char *args)
{
#ifdef EXEC_COMMAND
