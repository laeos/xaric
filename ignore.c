 ...
 */
void cmd_ignore(struct command *cmd, char *args)
{
    char *nick, *type;
...
int check_ignore(char *nick, char *userhost, char *channel, long type, char *str)
{
	...
	if (channel && is_channel(channel)
	    && (tmp = (Ignore *) list_lookup((struct list **) &ignored_nicks, channel, USE_WILDCARDS, !REMOVE_FROM_LIST))) {
	    if (tmp->dont & type)
		return (DONT_IGNORE);
	    if (tmp->type & type)
		return (IGNORED);
...
void cmd_tignore(struct command *cmd, char *args)
{
    ignore_list(NULL);
    if (ignored_nicks) {
	if (args && *args)
