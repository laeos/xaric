...
char zero_str[] = "0", one_str[] = "1", space_str[] = " ", on_str[] = "ON", off_str[] = "OFF", empty_str[] = "";
...
char *invite_channel = NULL,	/* last channel of an INVITE */
    ...
    *line_thing = NULL,		/* show_numeric_str, say( ), put at begin of line */
    oper_command = 0,		/* true just after an oper() command is given.  Used to tell the difference between an incorrect
				 * password generated by an oper() command and one generated when connecting to a new server */
    def_hostname[NAME_LEN+1],	/* gethostname() return */
    *local_host_name = NULL;	/* host the user would like to use (-H, /hostname) */

int irc_port = IRC_PORT,	/* port of ircd */
    strip_ansi_in_echo,		/* used in the send_text() */
    current_on_hook = -1, current_numeric,	/* this is negative of the current numeric! */
    key_pressed = 0, waiting_out = 0,	/* used by /WAIT command */
    waiting_in = 0,		/* used by /WAIT command */
    who_mask = 0,		/* keeps track of which /who switchs are set */
    away_set = 0,		/* set if there is an away message anywhere */
#ifdef HAVE_SSL
    do_use_ssl = 0,
#endif
...
static void parse_args(int argc, char *argv[])
{
	...
	case 'H':		/* Set local hostname, for vhost stuff */
	    malloc_strcpy(&local_host_name, optarg);
	    break;
...
static void load_xaric_environment(void)
{
    char *ptr;
...
static void load_xaric_servers(void)
{
...
void irc_exit(char *reason, char *formated)
{
    ...
    clean_up_processes();
    cursor_to_input();		/* Needed so that ircII doesn't gobble the last line of the kill. */
    term_cr();
    term_clear_to_eol();
    term_reset();
...
static void xaric_init(int argc, char *argv[])
{
    get_user_info();
...
extern void get_line_return(char unused, char *not_used)
{
    ...
    /* 
     * If we're NOT the main() call, then undo all that we messed up coming in. If stuff->done gets set to 1 when recursive_call is
     * zero, then something is VERY wrong. We can set stuff->prev->next to null because the call to get_line() holds a pointer to
     * stuff, so when it unrecurses, it will free it. */
    if (stuff->func) {
	not_used = NULL;
    ...
    /* We cant delete stuff here becuase the get_line function still needs to look at stuff->done.  So we let it delete the items off
     * the list.  But we removed it from the list, so we wont accidentally use it later. */
    return;
}
...
void get_line(char *prompt, int new_input, void (*func) (char, char *))
{
    ...
    /* ok.  we call io() until the user presses return, ending the input line.  get_line_return will then set get_line_done to one,
     * and we will stop getting characters and drop out. get_line_done NEVER sets this to one if we are in our call from main().
     * NEVER. */
    while (!stuff->done)
	io("get line");
    ...
    /* By the time we get here, stuff->done has been set to 1, which means that get_line_return has already freed the interesting
...
void io(const char *what)
{
	...
	last_warn = level;
	yell("io's nesting level is [%d],  [%s]<-[%s]<-[%s]<-[%s]<-[%s]<-[%s]", level, what, caller[level - 1], caller[level - 2],
    ...
    /* if the time changes, now - idle_time can be negative, and cause all kinds of problems */
    if (now > idle_time)
