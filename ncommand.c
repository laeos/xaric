...
void cmd_about(struct command *cmd, char *args)
{
...
void cmd_alias(struct command *cmd, char *args)
{
    char *str;
    char *command;
...
static void read_away_log(char *stuff, char *line)
{
...
void cmd_back(struct command *cmd, char *args)
{
    char *tmp = NULL;
    int minutes = 0;
    int hours = 0;
	...
	if (get_fset_var(FORMAT_BACK_FSET) && get_int_var(SEND_AWAY_MSG_VAR)) {
	    bitchsay("You were /away for %i hours %i minutes and %i seconds.", hours, minutes, seconds);
		...
		send_to_server(SERVER(from_server), "PRIVMSG %s :ACTION %s", chan->channel,
			       stripansicodes(convert_output_format(get_fset_var(FORMAT_BACK_FSET), "%s %d %d %d %d %s",
								    ...
								    get_int_var(MSGCOUNT_VAR),
								    args ? args : get_server_away(curr_scr_win->server))));
	    }
	}
...
void cmd_chwall(struct command *cmd, char *args)
{
    char *channel = NULL;
    char *chops = NULL;
    char *include = NULL;
	...
	if (i) {
	    put_it("%s",
		   convert_output_format(get_fset_var(FORMAT_BWALL_FSET), "%s %s %s %s %s", update_clock(GET_TIME), channel, "*", "*",
					 args));
	    if (exclude) {
...
void cmd_clear_tab(struct command *cmd, char *args)
{
    clear_array(&tabkey_array);
}

void cmd_ctcp_version(struct command *cmd, char *args)
{
    char *person;
    int type = 0;
...
void cmd_ctcp(struct command *cmd, char *args)
{
    char *to;
    char *stag = NULL;
    int tag;
	...
	if ((type = in_ctcp()) == -1)
	    ...
	    put_it("%s", convert_output_format(get_fset_var(FORMAT_SEND_CTCP_FSET),
					       "%s %s %s %s", update_clock(GET_TIME), to, stag ? stag : "VERSION",
					       args ? args : empty_str));
	}
    } else
	userage(cmd->name, cmd->qhelp);
...
void cmd_clear(struct command *cmd, char *args)
{
    char *arg;
    int all = 0, unhold = 0;
...
void cmd_cycle(struct command *cmd, char *args)
{
    char *to = NULL;
...
static void handle_dcc_chat(WhoisStuff * stuff, char *nick, char *args)
{
...
void cmd_dcc_chat(struct command *cmd, char *args)
{
...
void cmd_dcc(struct command *cmd, char *args)
{
    if (*args)
	process_dcc(args);
    else
...
void cmd_describe(struct command *cmd, char *args)
{
    char *target;
	...
	if (do_hook(SEND_ACTION_LIST, "%s %s", target, message))
	    put_it("%s", convert_output_format(get_fset_var(FORMAT_SEND_ACTION_OTHER_FSET),
					       "%s %s %s %s", update_clock(GET_TIME), get_server_nickname(from_server), target,
					       message));
...
void cmd_disconnect(struct command *cmd, char *args)
{
    char *server;
    char *message;
    int i;
	...
	put_it("%s",
	       convert_output_format(get_fset_var(FORMAT_DISCONNECT_FSET), "%s %s %s", update_clock(GET_TIME),
				     "Disconnecting from server", server_list[i].itsname));
...
void cmd_echo(struct command *cmd, char *args)
{
...
void cmd_flush(struct command *cmd, char *args)
{
...
void cmd_join(struct command *cmd, char *args)
{
	...
	if (my_strnicmp(chan, "-i", 2) == 0) {
		...
		/* XXX - right here we want to check to see if the channel is bound to this window.  if it is, we set it as the
		 * default channel.  If it is not, we warn the user that we cant do it */
		if (is_bound_anywhere(buffer) && !(is_bound_to_window(curr_scr_win, buffer)))
...
void cmd_linklook(struct command *cmd, char *args)
{
	...
	if (serv->status & SPLIT) {
	    if (!count)
		put_it("%s",
		       convert_output_format(get_fset_var(FORMAT_NETSPLIT_HEADER_FSET), "%s %s %s %s", "time", "server", "uplink",
					     "hops"));
	    if (do_hook(LLOOK_SPLIT_LIST, "%s %s %d", serv->name, serv->link, serv->hopcount))
		put_it("%s",
		       convert_output_format(get_fset_var(FORMAT_NETSPLIT_FSET), "%s %s %s %d", serv->time, serv->name, serv->link,
					     serv->hopcount));
	    count++;
	}
	serv = serv->next;
...
void cmd_map(struct command *cmd, char *args)
{
...
void cmd_me(struct command *cmd, char *args)
{
    if (args && *args) {
	char *target = NULL;
...
static void userhost_nsl(WhoisStuff * stuff, char *nick, char *args)
{
...
void cmd_nslookup(struct command *cmd, char *args)
{
    char *host;
...
void cmd_nwhowas(struct command *cmd, char *args)
{
    show_whowas();
}

void cmd_part(struct command *cmd, char *args)
{
    char *channel = NULL;
    struct channel *chan;
    int server = from_server;
...
void cmd_ping(struct command *cmd, char *args)
{
    struct timeval tp;
    char *to;
	...
	put_it("%s", convert_output_format(get_fset_var(FORMAT_SEND_CTCP_FSET), "%s %s %s", update_clock(GET_TIME), to, "PING"));
    }

}

void cmd_privmsg(struct command *cmd, char *args)
{
    char *nick;
...
void cmd_query(struct command *xcmd, char *args)
{
    char *nick, *rest;
    char *cmd = NULL;
...
void cmd_quit(struct command *cmd, char *args)
{
    int old_server = from_server;
    char *Reason;
    DCC_list *Client;
	...
	if (((Client->flags & DCC_TYPES) == DCC_REGETFILE) || ((Client->flags & DCC_TYPES) == DCC_FILEREAD)
	    || ((Client->flags & DCC_TYPES) == DCC_FILEOFFER) || ((Client->flags & DCC_TYPES) == DCC_RESENDOFFER))
	    active_dcc++;
...
void cmd_quote(struct command *cmd, char *args)
{
    if (!in_on_who && !doing_privmsg && args && *args)
	send_to_server(SERVER(from_server), "%s", args);
}

void cmd_reconnect(struct command *cmd, char *args)
{
    char scommnd[6];
...
void cmd_reset(struct command *cmd, char *args)
{
...
void cmd_hook(struct command *cmd, char *args)
{
    if (args && *args)
...
void cmd_send_text(struct command *cmd, char *args)
{
    char *tmp;
    char *c = NULL;
...
void cmd_server(struct command *cmd, char *args)
{
    char *server = NULL;
    int i;
...
void cmd_setenv(struct command *cmd, char *args)
{
    char *env_var;
...
void cmd_sping(struct command *cmd, char *args)
{
...
void cmd_squit(struct command *cmd, char *args)
{
...
void cmd_stats(struct command *cmd, char *args)
{
...
void cmd_showidle(struct command *cmd, char *args)
{
    struct channel *tmp;
    char *channel = NULL;
    int count = 0;
...
void cmd_unalias(struct command *cmd, char *args)
{
    char *c;
...
static void userhost_ignore(WhoisStuff * stuff, char *nick1, char *args)
{
    char *p, *arg;
	...
	while (igptr != NULL) {
	    igtmp = igptr->next;
	    if (match(igptr->nick, ignorebuf) || match(nick, igptr->nick)) {
...
void cmd_doig(struct command *cmd, char *args)
{
    char *nick;
...
void cmd_ison(struct command *cmd, char *args)
{
...
void cmd_ircii_version(struct command *cmd, char *args)
{
    char *host;
...
void cmd_userhost(struct command *cmd, char *args)
{
...
void cmd_users(struct command *cmd, char *args)
{
    struct channel *chan;
    ...
    *modebuf = 0;
	...
	sprintf(modebuf, "%s!%s", nicks->nick, nicks->host ? nicks->host : "<UNKNOWN@UNKOWN>");
	if (match(spec, modebuf) && (!ops || ((ops == 1) && nicks->chanop) || ((ops == 2) && !nicks->chanop))) {
	    if (msg == 3)
		count--;
	    else if (msg == 4) {
		if (!isme(nicks->nick))
		    send_to_server(SERVER(server), "KILL %s :%s (%i", nicks->nick,
				   args && *args ? args : get_reason(nicks->nick), count + 1);
		else
		    count--;
	    } else if (msg == 5) {
		if (!isme(nicks->nick))
		    send_to_server(SERVER(server), "KICK %s %s :%s", chan->channel,
				   nicks->nick, (args && *args) ? args : get_reason(nicks->nick));
		else
		    count--;
	    } else if (msg == 6) {
		if (my_stricmp(nicks->nick, get_server_nickname(from_server))) {
			...
			put_it("%-10s  %4d   %4d   %4d     %4d",
			       nicks->nick, nicks->dopcount, nicks->kickcount, nicks->nickcount, nicks->floodcount);
		}
	    } else if (msg == 1 || msg == 2) {
		if (count)
		    strcat(msgbuf, ",");
		...
		if (!count
		    && do_hook(USERS_HEADER_LIST, "%s %s %s %s %s %s %s", "Level", "aop", "prot", "Channel", "Nick", "+o", "UserHost"))
		...
		if ((hook = do_hook(USERS_LIST, "%d %d %d %s %s %s %c",
				    0, 0, 0, chan->channel, nicks->nick, nicks->host, nicks->chanop ? '@' : ' '))) {
	...
	put_it("%s",
	       convert_output_format(get_fset_var((msg == 1) ? FORMAT_SEND_MSG_FSET : FORMAT_SEND_NOTICE_FSET), "%s %s %s",
				     update_clock(GET_TIME), msgbuf, args));
...
void cmd_whois_lm(struct command *cmd, char *args)
{
    if (recv_nick)
...
void cmd_whois_i(struct command *cmd, char *args)
{
    char *channel = NULL;
	...
	send_to_server(SERVER(from_server), "WHOIS %s %s", get_server_nickname(from_server), get_server_nickname(from_server));
}

void cmd_whois(struct command *cmd, char *args)
{
    if (args && *args)
...
void cmd_wholeft(struct command *cmd, char *args)
{
    show_wholeft(NULL);
}

void cmd_whowas(struct command *cmd, char *args)
{
    char *stuff = NULL;
...
struct command xaric_cmds[] = {
    ...
    {"BANTYPE", NULL, NULL, cmd_bantype,
     "%W/%nbantype %Y<%nNormal%G|%nBetter%G|%nHost%G|%nDomain%G |%nScrew%Y>%n\n- When a ban is done on a nick, it uses %Y<%nbantype%Y>%n"},
    ...
    {"CLOSE", NULL, NULL, cmd_oper_stuff,
     "Requires irc operator status. Close any connections from clients who have not fully registered yet."},
    ...
    {"HOSTNAME", NULL, NULL, cmd_hostname,
    ...
    {"NOTICE", "NOTICE", NULL, cmd_privmsg,
...
void send_text(char *nick_list, char *text, char *command, int hook, int log)
{
	...
	if (hook && do_hook(target[i].hook_type, "%s %s", target[i].nick_list, target[i].message)) {
		...
		put_it("%s", convert_output_format(target[i].output,
						   "%s %s %s %s", update_clock(GET_TIME), target[i].nick_list,
						   get_server_nickname(from_server), copy));
	    else
 ...
 * executed). 
 */
int parse_command(char *line, int hist_flag, char *sub_args)
{
...
BUILT_IN_COMMAND(load)
{
	...
	/* 
	 * If we use the args flag, then we will get the next filename (via continue) but at the bottom of the loop we will exit the
	 * loop */
	if (my_strnicmp(filename, "-args", strlen(filename)) == 0) {
		...
		/* uzfopen emits an error if the file is not found, so we dont have to. */
		status_update(1);
		load_depth--;
		new_free(&expanded);
		return;
...
/* *_NOT_* attached, so dont "fix" it */
		...
		for (;; line++) {
			 ...
			 */
			/* 
			 * If we have \\ at the end of the line, that should indicate that we DONT want the slash to escape the
			 * newline (hop)
			 * 
			 * We cant just do start[len-2] because we cant say what will happen if len = 1... (a blank line)
			 * 
			 * SO.... If the line ends in a newline, and If there are at least 2 characters in the line and the 2nd to
			 * the last one is a \ and, If there are EITHER 2 characters on the line or the 3rd to the last character is
			 * NOT a \ and, If the line isnt too big yet and, If we can read more from the file, THEN -- adjust the length 
			 * of the string */
			while ((start[len - 1] == '\n') &&
			       (len >= 2 && start[len - 2] == '\\') &&
			       (len < 3 || start[len - 3] != '\\') &&
			       (len < (BIG_BUFFER_SIZE / 2)) && (fgets(&(start[len - 2]), (BIG_BUFFER_SIZE / 2) - len, fp))) {
			    len = strlen(start);
			    line++;
			}
			...
			while (start && *start) {
			    ...
			    while ((ptr = sindex(optr, "{};/")) && ptr != optr && ptr[-1] == '\\')
				optr = ptr + 1;
				...
				if (!paste_level) {
				    parse_line(NULL, current_row, flag ? args : get_int_var(INPUT_ALIASES_VAR) ? empty_str : NULL, 0,
					       0);
				...
				*ptr = c;
				...
				case '/':
				    {
					/* If we're in a comment, any slashes that arent preceeded by a star is just ignored (cause
					 * its in a comment, after all >;) */
					if (in_comment) {
					...
					/* We're not in a comment... should we start one? */
						...
						/* This hack (at the request of Kanan) makes it so you can only have comments at the
						 * BEGINNING of a line, and any midline comments are ignored. This is required to run
						 * lame script packs that do ascii art crap (ie, phoenix, textbox, etc) */
					    {
						...
						if (ptr[1] == '\0') {
							...
							parse_line(NULL, current_row,
								   flag ? args : get_int_var(INPUT_ALIASES_VAR) ? empty_str : NULL, 0,
								   0);
							new_free(&current_row);
							ack = 0;	/* no semicolon.. */
						    } else
				...
				case ';':
					...
					if ((*(ptr + 1) == '\0') && (!paste_level)) {
					    parse_line(NULL, current_row,
						       flag ? args : get_int_var(INPUT_ALIASES_VAR) ? empty_str : NULL, 0, 0);
					    new_free(&current_row);
					} else
					    malloc_strcat(&current_row, ";");
					no_semicolon = 1;
		...
		if (current_row) {
		    if (paste_level)
			yell("Unexpected EOF in %s trying to match '{' at line %d", expanded, paste_line);
		    else
...
/* End of reformatting */
	...
	if (flag)
	    break;
    }				/* end of the while loop that allows you to load more then one file at a time.. */
    status_update(1);
    load_depth--;
