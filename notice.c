...
static void parse_server_notice(char *from, char *line)
{
    ...
    if (!from || !*from)
	from = server_list[from_server].itsname ? server_list[from_server].itsname : server_list[from_server].name;
	...
	if (from && *from && strcmp(get_server_itsname(from_server), from)) {
		...
		if (no_flooding) {
			...
			if (do_hook(type, "%s %s", from, line)) {
			    ...
			    put_it("%s",
				   convert_output_format(get_fset_var(FORMAT_BWALL_FSET), "%s %s %s %s %s", update_clock(GET_TIME), q,
							 from, FromUserHost, newline));
			}
			...
			if (do_hook(type, "%s %s", from, line))
			    put_it("%s",
				   convert_output_format(get_fset_var(FORMAT_NOTICE_FSET), "%s %s %s %s", update_clock(GET_TIME), from,
							 FromUserHost, newline));
		    } else {
			if (do_hook(type, "%s %s %s", from, to, line))
			    put_it("%s",
				   convert_output_format(get_fset_var(FORMAT_PUBLIC_NOTICE_FSET), "%s %s %s %s %s",
							 update_clock(GET_TIME), from, FromUserHost, to, newline));
 ...
 */
extern void got_initial_version(char **ArgList)
{
    char *server, *version;
