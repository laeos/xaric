...
void cmd_notify(struct command *cmd, char *args)
{
    char *nick, *ptr;
    int no_nicks = 1;
	...
	while (nick) {
		...
		*ptr++ = '\0';
		...
		if (*nick) {
			...
			for (servnum = 0; servnum < number_of_servers; servnum++) {
			    if ((new =
				 (NotifyList *) remove_from_list((struct list **) &server_list[servnum].notify_list, nick)) != NULL) {
				new_free(&(new->nick));
				new_free(&(new->user));
				new_free(&(new->host));
				new_free(&new);
...
void do_serv_notify(int servnum)
{
    int size;
 ...
 *
 */
void notify_mark(char *nick, char *user, char *host, int flag)
{
    NotifyList *tmp;
    ...
    if ((tmp =
	 (NotifyList *) list_lookup((struct list **) &server_list[from_server].notify_list, nick, !USE_WILDCARDS,
	...
	if (flag) {
		...
		put_it("%s",
		       convert_output_format(get_fset_var(FORMAT_NOTIFY_SIGNON_UH_FSET), "%s %s %s %s", update_clock(GET_TIME), nick,
					     tmp->user, tmp->host));
	    }
	} else {
	    if (tmp->flag) {
		put_it("%s",
		       convert_output_format(get_fset_var(FORMAT_NOTIFY_SIGNOFF_UH_FSET), "%s %s %s %s", update_clock(GET_TIME), nick,
					     tmp->user, tmp->host));
