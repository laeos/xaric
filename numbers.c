 ...
 */
static char *numeric_banner(void)
{
    static char thing[4];
...
int check_sync(int comm, char *channel, char *nick, char *whom, char *bantime, struct channel *chan)
{
...
static int check_server_sync(char *from, char **ArgList)
{
	...
	if (!match(from, get_server_name(from_server))) {
		...
		put_it("%s",
		       convert_output_format(get_format(FORMAT_DESYNC_FSET), "%s %s %s", update_clock(GET_TIME), ArgList[0], from));
	    return 1;
	}
    }
    return 0;
...
void display_msg(char *from, char **ArgList)
{
    ...
    if (from && (my_strnicmp(get_server_itsname(from_server), from, strlen(get_server_itsname(from_server))) == 0))
	from = NULL;
    if ((ptr = strchr(rest, ':')))
	*ptr++ = 0;
...
/*                strlen(rest) && ptr ? ":"      : empty_str, */
	   ...
	   ptr ? ptr : empty_str, drem ? "(from " : empty_str, drem ? from : empty_str, drem ? ")" : empty_str);
}
...
static void not_valid_channel(char *from, char **ArgList)
{
	...
	put_it("%s",
	       convert_output_format(get_format(FORMAT_SERVER_MSG2_FSET), "%s %s %s", update_clock(GET_TIME), channel, ArgList[1]));
    }
}
...
static int handle_server_ping(int comm, char *from, char **ArgList)
{
#ifdef HAVE_GETTIMEOFDAY
...
static int handle_server_stats(char *from, char **ArgList, int comm)
{
	...
	put_it("%s",
	       convert_output_format("$G %K[%ntotal servers on irc%K(%n\002$0\002%K)]%n %K(%navg. $1 users per server%K)", "%d %d",
				     servers, (servers) ? (int) (total_users / servers) : 0));

	put_it("%s",
	       convert_output_format("$G %K[%ntotal channels created%K(%n\002$0\002%K)]%n %K(%navg. $1 users per channel%K)", "%d %d",
				     chans, (chans) ? (int) (total_users / chans) : 0));
	break;
    case 250:
	{
	    char *p;
		...
		p++;
		put_it("%s",
		       convert_output_format("$G %K[%nHighest client connection count%K(%n\002$0\002%K) %K(%n\002$1\002%K)]%n",
					     "%s %s", ArgList[3], p));
	    }
	    break;
	}
...
static void whohas_nick(WhoisStuff * stuff, char *nick, char *args)
{
...
void numbered_command(char *from, int comm, char **ArgList)
{
	...
	if (ArgList[1]) {
		...
		put_it("%s",
		       convert_output_format(get_format(FORMAT_INVITE_USER_FSET), "%s %s %s", update_clock(GET_TIME), ArgList[0],
					     ArgList[1]));
	}
	break;
	...
	if (!is_server_connected(from_server))
		...
		put_it("%s", convert_output_format(get_format(FORMAT_381_FSET), "%s %s %s", update_clock(GET_TIME), from, *ArgList));
	    else
		display_msg(from, ArgList);
	}
	...
	if (oper_command) {
	    ...
	    say("Password required for connection to server %s", get_server_name(from_server));
	    close_server(from_server, empty_str);
	    strcpy(server_num, ltoa(from_server));
	    add_wait_prompt("Server Password:", password_sendline, server_num, WAIT_PROMPT_LINE);
	}
	break;
    ...
    case 368:			/* #define RPL_ENDOFBANLIST 368 */
	{
		...
		if (do_hook(current_numeric, "%s %d %s", from, number_of_bans, *ArgList)) {
		    put_it("%s Total Number of Bans on %s [%d]", numeric_banner(), ArgList[0], number_of_bans);
		}
	    }
	    number_of_bans = 0;
	    break;
	...
	if (comm == 364) {
		...
		if (get_format(FORMAT_LINKS_FSET)) {
		    put_it("%s",
			   convert_output_format(get_format(FORMAT_LINKS_FSET), "%s %s %s", ArgList[0], ArgList[1],
						 ArgList[2] ? ArgList[2] : "0"));
		    break;
		}
	    }
	}
...
/*              case 242:                #define RPL_STATSUPTIME      242 */
		PasteArgs(ArgList, 0);
		if (from && !my_strnicmp(get_server_itsname(from_server), from, strlen(get_server_itsname(from_server))))
		    from = NULL;
		if (from)
	...
	case 329:		/* #define CREATION_TIME 329 */
		...
		message_from(ArgList[0], LOG_CRAP);
		put_it("%s Channel %s was created at %s", numeric_banner(), ArgList[0], this_sucks);
	...
	case 351:		/* #define RPL_VERSION 351 */
		...
		PasteArgs(ArgList, 2);
		put_it("%s",
		       convert_output_format(get_format(FORMAT_SERVER_FSET), "Server %s %s %s", ArgList[0], ArgList[1], ArgList[2]));
	    }
	    break;
	...
	case 364:		/* #define RPL_LINKS 364 */
		...
		if (ArgList[2]) {
		    PasteArgs(ArgList, 2);
		    put_it("%s %-20s %-20s %s", numeric_banner(), ArgList[0], ArgList[1], ArgList[2]);
		} else {
		    PasteArgs(ArgList, 1);
	...
	case 376:		/* #define RPL_ENDOFMOTD 376 */
	    if (get_int_var(SHOW_END_OF_MSGS_VAR) && (!get_int_var(SUPPRESS_SERVER_MOTD_VAR) || !get_server_motd(from_server))) {
		PasteArgs(ArgList, 0);
		put_it("%s %s", numeric_banner(), ArgList[0]);
	    }
