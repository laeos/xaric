...
void cmd_remove_log(struct command *cmd, char *args)
{
    char *expand;
    char *filename = NULL;
...
void cmd_readlog(struct command *cmd, char *args)
{
    char *expand;
...
static void log_prompt(char *name, char *line)
{
	...
	add_wait_prompt("*** Hit any key for more, 'q' to quit ***", log_prompt, name, WAIT_PROMPT_KEY);
    } else {
	if (msg_fp)
	    fclose(msg_fp);
	set_msg_screen(NULL);
