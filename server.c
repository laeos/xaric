 ...
 */
void 
set_server_bits (fd_set * rd, fd_set * wr)
{
	int i;
 ...
 */
extern int 
find_in_server_list (char *server, int port)
{
	int i, len, len2;
 ...
 */
int 
find_server_refnum (char *server, char **rest)
{
	int refnum;
	int port = irc_port;
 ...
 */
void 
add_to_server_list (char *server, int port, char *password, char *nick, int overwrite)
{
 ...
 */
void 
parse_server_info (char *name, char **port, char **password, char **nick)
{
	char *ptr;
 ...
 */
static int 
connect_to_server_direct (char *server_name, int port)
{
	int new_des;
...
int 
connect_to_server_by_refnum (int refnum, int c_server)
{
	char *sname;
	int sport;
	int conn;
 ...
 */
int 
read_server_file (char *servers_file)
{
	FILE *fp;
	int some = 0;
	char *file_path = NULL;
...
#if 0
void 
MarkAllAway (char *command, char *message, char *subargs)
{
	int old_server;
 ...
 */
char *
set_server_password (int ssp_index, char *password)
{
 ...
 */
void 
set_server_away (int ssa_index, char *message)
{
...
void 
set_server_flag (int ssf_index, int flag, int value)
{
	if (ssf_index == -1)
...
int 
get_server_flag (int gsf_index, int value)
{
	if (gsf_index == -1)
 ...
 */
void 
set_server_version (int ssv_index, int version)
{
	if (ssv_index == -1)
 ...
 */
int 
get_server_version (int gsv_index)
{
	if (gsv_index == -1)
...
char *
get_server_itsname (int gsi_index)
{
	if (gsi_index == -1)
...
void 
set_server_itsname (int ssi_index, char *name)
{
	if (ssi_index == -1)
 ...
 */
int 
is_server_connected (int isc_index)
{
...
WhoisQueue *
get_server_qhead (int gsq_index)
{
	if (gsq_index == -1)
		return WQ_head;
...
WhoisStuff *
get_server_whois_stuff (int gsw_index)
{
	if (gsw_index == -1)
...
WhoisQueue *
get_server_qtail (int gsq_index)
{
	if (gsq_index == -1)
		return WQ_tail;
	else if (gsq_index < number_of_servers)
...
void 
set_server_qhead (int ssq_index, WhoisQueue * value)
{
	if (ssq_index == -1)
		WQ_head = value;
...
void 
set_server_qtail (int ssq_index, WhoisQueue * value)
{
	if (ssq_index == -1)
		WQ_tail = value;
	else if (ssq_index < number_of_servers)
 ...
 */
void 
set_server2_8 (int ss2_index, int value)
{
 ...
 */
int 
get_server_operator (int gso_index)
{
 ...
 */
void 
set_server_operator (int sso_index, int flag)
{
 ...
 */
void 
set_server_nickname (int ssn_index, char *nick)
{
...
void 
register_server (int ssn_index, char *nick)
{
	int old_from_server = from_server;
...
void 
set_server_motd (int ssm_index, int flag)
{
...
void 
server_is_connected (int sic_index, int value)
{
...
/* send_to_server: sends the given info the the server */
void 
my_send_to_server (int server, const char *format,...)
{
	char buffer[BIG_BUFFER_SIZE + 1];	/* make this buffer *much*
						 * bigger than needed */
	char *buf = buffer;
 ...
 */
void 
change_server_nickname (int ssn_index, char *nick)
{
	int old_from_server = from_server;
...
void 
accept_server_nickname (int ssn_index, char *nick)
{
	malloc_strcpy (&server_list[ssn_index].nickname, nick);
...
char *
get_pending_nickname (int servnum)
{
