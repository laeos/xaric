...
static void 
add_mode_buffer (char *buffer, int mode_str_len_p)
{
...
static char *
ban_it (char *nick, char *u, char *host)
{
...
userhost_ban (WhoisStuff * stuff, char *nick1, char *args)
{
	...
	char *host = NULL, *nick = NULL, *u = NULL;
	WhowasList *whowas = NULL;
	...
	if (!stuff || !stuff->nick || !nick1 || !strcmp (stuff->user, "<UNKNOWN>") || my_stricmp (stuff->nick, nick1))
	{
		if (nick1 && channel && (whowas = check_whowas_nick_buffer (nick1, channel, 0)))
		{
			...
			u = m_strdup (clear_server_flags (whowas->nicklist->host));
	...
	{
		nick = stuff->nick;
		u = m_strdup (clear_server_flags (stuff->user));
		host = stuff->host;
	}
...
void
cmd_kickban (struct command *cmd, char *args)
{
	...
	for (nicks = next_nicklist (chan, NULL); nicks; nicks = next_nicklist (chan, nicks))
	{
		if (my_stricmp (spec, nicks->nick) == 0)
		{
			...
			u = clear_server_flags (t);
			host = strchr (u, '@');
			*host++ = 0;

			send_to_server ("MODE %s -o+b %s %s", chan->channel, nicks->nick, ban_it (nicks->nick, u, host));
	...
	if (count == 0)
		add_to_userhost_queue (spec, userhost_ban, "%s %s %s", chan->channel, spec, rest ? rest : get_reason (nicks ? nicks->nick : NULL));
}
...
static void 
set_default_bantype (char value, char *helparg)
{
	int bant;
	...
	bitchsay ("BanType set to %s", (bant == 1) ? "\002N\002ormal" : (bant == 2) ? "\002B\002etter" : (bant == 3) ? "\002H\002ost" : (bant == 4) ? "\002D\002omain" : (bant == 5) ? "\002U\002ser" : "\002S\002crew");
	set_int_var (BANTYPE_VAR, bant);
}

void
cmd_bantype (struct command *cmd, char *args)
{
	if (args && *args)
...
void
cmd_deop (struct command *cmd, char *args)
{
	char *to = NULL, *temp;
	int count, max;
	ChannelList *chan;
...
void
cmd_oper (struct command *cmd, char *args)
{
	char *password;
	char *nick;
...
void
cmd_unkey (struct command *cmd, char *args)
{
	char *channel = NULL;
	int server = from_server;
	ChannelList *chan;
