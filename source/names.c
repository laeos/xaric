		...
		if (!(new = find_nicklist_in_channellist (nick, chan, 0)))
		{
			...
			if (!(whowas = check_whowas_buffer (nick, userhost ? userhost : "<UNKNOWN>", channel, 1)))
			{
				...
				new->idle_time = new->kicktime =
					...
					new->floodtime = new->bantime = time (NULL);
 ...
 */
char *
compress_modes (int server, char *channel, char *modes)
{
...
switch_channels (char key, char *ptr)
{
	...
	if (server_list[from_server].chan_list)
	{
		...
		{
			if ((tmp->server == from_server) && !is_current_channel (tmp->channel, from_server, 0) && /* !(is_bound(tmp->channel, from_server)) && */  curr_scr_win == tmp->window)
			{
				set_current_channel_by_refnum (0, tmp->channel);
				update_all_windows ();
				update_all_status (curr_scr_win, NULL, 0);
...
void 
remove_from_join_list (char *chan, int server)
{
...
int 
get_win_from_join_list (char *chan, int server)
{
	struct joinlist *tmp;
	int found = 0;
...
char *
get_chan_from_join_list (int server)
{
	struct joinlist *tmp;
	char *found = NULL;
...
int 
in_join_list (char *chan, int server)
{
	struct joinlist *tmp;
...
int 
got_info (char *chan, int server, int type)
{
	struct joinlist *tmp;
...
void 
add_to_join_list (char *chan, int server, int winref)
{
	struct joinlist *tmp;
...
static void 
add_to_mode_list (char *channel, int server, char *mode)
{
...
static void 
check_mode_list_join (char *channel, int server)
{
	struct modelist *mptr;
