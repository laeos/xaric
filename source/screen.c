 ...
 */
void 
add_wait_prompt (char *prompt, void (*func) (char *, char *), char *data, int type)
{
...
add_to_display_list (Window * window, const unsigned char *str)
{
	...
	/* Hmmm.... display_ip /really/ shouldnt be null here, I think the window struct was
	   free'ed before this call. D'oh! */
...
static void 
display_color (long color1, long color2)
{
	char seq[32];
 ...
 */
int 
is_cursor_in_display (Screen * screen)
{
 ...
 */
void 
repaint_window (Window * w, int start_line, int end_line)
{
...
void 
set_screens (fd_set * rd, fd_set * wd)
{
	Screen *screen;
