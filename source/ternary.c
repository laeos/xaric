...
struct tnode *
t_insert (struct tnode *p, char *string, struct command *data)
{
	if (p == NULL)
	{
...
struct tnode *
t_remove (struct tnode *p, char *s, struct command **data)
{
	if (!p)
	{
		(*data) = NULL;
...
static inline struct command *
t_finish_search (struct tnode *p)
{
	int mask;
	...
	/* if not, we need to start our search on the NEXT child.
	   remeber, *s == splitchar in t_search :) */
...
struct command *
t_search (struct tnode *root, char *s)
{
	struct tnode *p;
...
struct tnode *
t_build (struct tnode *root, struct command c[], int n)
{
	int m;
...
static void
local_traverse (struct tnode *p, void (*fcn) (struct command *, char *), char *data)
{
	if (!p)
		return;
	if (p->data)
...
void
t_traverse (struct tnode *p, char *s, void (*fcn) (struct command *, char *), char *data)
{
	while (p)
	{
		if (*s == p->splitchar)
